...
```{r}
processed.data <- read.csv("../data/2022-07-25_tumor-size_R-formatted.csv") %>%
...
final.processed <- 
  ...
  mutate(experiment = fct_relevel(experiment, "mimic10","mimic11","mimic12", after = 9)) %>%
...
```{r mimic1,6,7 filtered final output}

H55.filter <-
  final.processed %>%
  filter(experiment %in% c("mimic1","mimic6","mimic7")) %>%
  mutate(patient = "HONC-60 55")
...
ggsave("../figures/HONC55-filtered-boxplot.png", width = 7, height = 8)


```

```{r mimic5 separate boxplot}

H102.filter <-
  final.processed %>%
  filter(experiment == "mimic5") %>%
  mutate(patient = "HONC-60 102")
...
H102.filter %>%
  ...
  xlab("Treatment") +
  ylab("Tumor Volume") +
  ggtitle("Mimic5 Tumor Volume by Intervention") +
  theme_bw()

ggsave("../figures/HONC102-filtered-boxplot.png", width = 7, height = 8)

```

```{r mimic10 separate boxplot}

H68.filter <-
  final.processed %>%
  filter(experiment == "mimic10") %>%
  mutate(patient = "HONC-60 68")
...
H68.filter %>%
  ...
  xlab("Treatment") +
  ylab("Tumor Volume") +
  ggtitle("Mimic10 Tumor Volume by Intervention") +
  theme_bw()

ggsave("../figures/HONC68-filtered-boxplot.png", width = 7, height = 8)

```

```{r mimic11 separate boxplot}

H79.filter <-
  final.processed %>%
  filter(experiment == "mimic11") %>%
  mutate(patient = "HONC-60 79",
...
H79.filter %>%
  ...
  xlab("Treatment") +
  ylab("Tumor Volume") +
  ggtitle("Mimic11 Tumor Volume by Intervention") +
  theme_bw()

ggsave("../figures/HONC79-filtered-boxplot.png", width = 7, height = 8)

```

```{r mim12 individual boxplot}
H85.filter <-
  final.processed %>%
  filter(experiment == "mimic12") %>%
  mutate(patient = "HONC-60 85") %>%
  mutate(treatment = fct_relevel(treatment, "IgG"))
...
H85.filter %>%
  ...
  xlab("Treatment") +
  ylab("Tumor Volume") +
  ggtitle("Mimic12 Tumor Volume by Intervention") +
  theme_bw()

ggsave("../figures/HONC85-filtered-boxplot.png", width = 7, height = 8)
