...
```{r}
tsize.files <- list.files(file.path(paths$box, 'data'), pattern = "AOT", full.names = T)[1:8]

tsize.file.2 <- unlist(lapply(tsize.files, function(x) list.files(x, pattern = "_size")))

tsize.raw <- lapply(1:8, function(x) read_excel(file.path(tsize.files[x], tsize.file.2[x])))
...
```{r}
write.csv(mimic.cleaned, "../data/2022-03-22_tumor-size_R-formatted.csv", row.names = F)
```